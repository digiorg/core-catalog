...
The decoded Dockerfile is validated against the pinned NGINX image with
`nginx -t`, bounded container readiness, and an exact `200 text/plain` response
test so KCL, Dockerfile, shell `printf`, and NGINX quoting are exercised together.
